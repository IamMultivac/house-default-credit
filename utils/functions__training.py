...
def lgbm_classification_learner(
    ...
    dataset_init_score: Optional[
        Union[List, List[List], np.ndarray, pd.Series, pd.DataFrame]
    ] = None,
) -> LearnerReturnType:
    """
    ...
    features = (
        features if not encode_extra_cols else expand_features_encoded(df, features)
    )
    ...
    dtrain = lgbm.Dataset(
        ...
        params={"linear_trees": linear_trees, "verbose": False},
    )
            ...
            if params["objective"] == "multiclass":
                ...
                shap_expected_value_multiclass = {
                    f"shap_expected_value_{class_index}": np.repeat(
                        expected_value, len(class_shap_values)
                    )
                ...
                shap_output = merge(
                    shap_values_multiclass, shap_expected_value_multiclass
                )
                ...
                shap_output = {
                    ...
                    "shap_expected_value": np.repeat(
                        shap_expected_value[1], len(shap_values)
                    ),
                }
    ...
    log = {
        "lgbm_classification_learner": {
            ...
            "feature_importance": dict(
                zip(features, bst.feature_importance().tolist())
            ),
...
def model_pipeline(
    ...
    init_score = params["learner_params"]["extra_params"].get(
        "dataset_init_score", None
    )

    if linear_trees:
        logger.info(
            f"linear trees will be applied so training time may increase significantly."
        )

    skf = StratifiedKFold(n_splits=cv, shuffle=True, random_state=random_state)
    train_df = train_df.reset_index(drop=True)
    logger.info(f"Starting pipeline: Generating {cv} k-fold training...")
    for i, (train_idx, validation_idx) in enumerate(
        skf.split(train_df.loc[:, features], train_df.loc[:, target_column])
    ):
        logger.info(f"Training for fold {i + 1}")
        tmp_train_df = train_df.loc[train_idx, :]
        tmp_validation = train_df.loc[validation_idx, :]

        tmp_bst, _, model_logs = lgbm_classification_learner(
            tmp_train_df,
            features=features,
    ...
    roc_auc_val = get_roc(
        validation_preds_df[target_column], validation_preds_df["prediction"]
    )

    log_loss_oof = get_logloss(oof_preds_df[target_column], oof_preds_df["prediction"])
    log_loss_val = get_logloss(
        validation_preds_df[target_column], validation_preds_df["prediction"]
    )

    logs = {
        "lgbm_classification_learner": {"model_logs": model_logs, "predict_fn": bst},
        "data": {"oof_df": oof_preds_df, "validation_df": validation_preds_df},
        "metrics": {
            "roc_auc": {"out_of_fold": roc_auc_oof, "validation": roc_auc_val},
            "log_loss": {"out_of_fold": log_loss_oof, "validation": log_loss_val},
        },
    }
